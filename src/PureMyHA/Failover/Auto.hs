...
executeFailover topo = runExceptT $ do
  ...
  let postEnv = HookEnv (ccName cc) (Just (unHostName (nodeHost candidateId))) oldSourceHost (Just "DeadSource") ts Nothing Nothing Nothing Nothing
...
runPreFailoverHook candidateId oldSourceHost = do
  ...
  ts <- liftIO getCurrentTimestamp
  let preEnv = HookEnv (ccName cc) (Just (unHostName (nodeHost candidateId))) oldSourceHost (Just "DeadSource") ts Nothing Nothing Nothing Nothing
...
promoteWithOnFailureHook candidateId waitTimeout oldSourceHost = do
  ...
  case promoteResult of
    Left err -> do
      ...
      ts <- liftIO getCurrentTimestamp
      mHooks <- getHooksConfig
      let failEnv = HookEnv clusterName (Just (unHostName (nodeHost candidateId))) oldSourceHost (Just "PromoteFailed") ts Nothing Nothing Nothing Nothing
...
fenceNode clusterName survivorHost ns = do
  ...
  case result of
    ...
    Right () -> do
      ...
      ts <- liftIO getCurrentTimestamp
      let hookEnv = HookEnv clusterName (Just survivorHost) (Just (unHostName (nodeHost nid))) Nothing ts Nothing Nothing Nothing Nothing
