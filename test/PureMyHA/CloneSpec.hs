...
spec = do
  ...
  describe "selectDonorAuto" $ do
    let replica2 = healthyReplica
        ...
        replica3 = mkNodeState (unsafeNodeId "db3" 3306) Replica
    ...
    it "auto-selects the replica with the highest GTID transaction count" $ do
      let nodes = Map.fromList
            ...
            , (unsafeNodeId "db2" 3306, replica2)
            , (unsafeNodeId "db3" 3306, replica3)
            ]
      selectDonorAuto nodes (unsafeNodeId "db1" 3306) `shouldBe` Right (unsafeNodeId "db3" 3306)
    ...
    it "excludes the recipient node from donor candidates" $ do
      ...
      selectDonorAuto nodes (unsafeNodeId "db3" 3306) `shouldBe` Right (unsafeNodeId "db2" 3306)
    ...
    it "returns Left when no reachable replica exists other than recipient" $ do
      ...
      selectDonorAuto nodes (unsafeNodeId "db2" 3306) `shouldSatisfy` isLeft

    it "excludes unreachable replicas from donor selection" $ do
      let unreachableDb3 = unreachableNode (unsafeNodeId "db3" 3306)
          nodes = Map.fromList
            [ (unsafeNodeId "db1" 3306, healthySource)
            , (unsafeNodeId "db2" 3306, replica2)
            , (unsafeNodeId "db3" 3306, unreachableDb3)
            ]
